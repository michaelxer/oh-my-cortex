...
import { WORKSTATE_CONTINUATION_PROMPT } from "./system-reminder-templates"
import { resolveRecentPromptContextForSession } from "./recent-model-resolver"
import type { SessionState } from "./types"

export type WorkStateContinuationResult = "injected" | "skipped_background_tasks" | "skipped_agent_unavailable" | "failed"

export async function injectWorkStateContinuation(input: {
  ctx: PluginInput
  sessionID: string
  planName: string
  remaining: number
...
}): Promise<WorkStateContinuationResult> {
  const {
    ctx,
    sessionID,
    planName,
  ...
  const preferredSessionContext = preferredTaskSessionId
    ...
    : ""
	const prompt =
		WORKSTATE_CONTINUATION_PROMPT.replace(/{PLAN_NAME}/g, planName) +
		`\n\n[Status: ${total - remaining}/${total} completed, ${remaining} remaining]` +
		preferredSessionContext +
		worktreeContext
	const continuationAgent = resolveRegisteredAgentName(
		agent ?? (isAgentRegistered("lead") ? "lead" : undefined),
	)
	...
	try {
		log(`[${HOOK_NAME}] Injecting workstate continuation`, { sessionID, planName, remaining })
    ...
    log(`[${HOOK_NAME}] WorkState continuation injected`, { sessionID })
    return "injected"
  } catch (err) {
    sessionState.promptFailureCount += 1
    sessionState.lastFailureAt = Date.now()
    log(`[${HOOK_NAME}] WorkState continuation failed`, {
      sessionID,
      error: String(err),
