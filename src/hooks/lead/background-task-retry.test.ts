...
import { createLeadHook } from "./lead-hook"
...
// Force process isolation in CI runner (globalThis.setTimeout override conflicts with other lead tests)
...
describe("lead background task retry", () => {
  let testDir: string
  ...
  beforeEach(() => {
    ...
    testDir = join(tmpdir(), `lead-background-retry-${randomUUID()}`)
  ...
  test("#given background tasks are still running #when retry fires before they finish #then lead keeps retrying until continuation can resume", async () => {
    // given
  ...
  test("#given multiple idle events arrive while background retry is already pending #when tasks are still running #then lead keeps only one retry timer active", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeWorkStateState(testDir, {
  ...
  test("#given background tasks keep running across multiple retries #when they finally finish on a later retry #then lead resumes exactly once", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeWorkStateState(testDir, {
  ...
  test("#given retry gate sees no running task but injector still does #when retry fires #then lead schedules another retry and does not advance cooldown", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeWorkStateState(testDir, {
  ...
  test("#given a persisted descendant becomes ineligible before retry fires #when retry runs #then lead re-checks descendant eligibility and does not inject", async () => {
    // given
    const descendantSessionID = "ses_descendant_retry_mismatch"
    setSessionAgent(descendantSessionID, "lead")
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeWorkStateState(testDir, {
      active_plan: planPath,
  ...
  test("#given continuation injection is already in flight #when another idle event arrives #then lead does not inject twice", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeWorkStateState(testDir, {
  ...
  test("#given a retry timer fires during an in-flight continuation that later fails #when the in-flight guard re-arms retry #then lead can recover on the next retry", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeWorkStateState(testDir, {
  ...
  test("#given a retry-driven continuation fails once #when retry handling re-arms the chain #then lead recovers on the next retry", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeWorkStateState(testDir, {
