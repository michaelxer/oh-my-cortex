...
import { isChiefPath } from "./chief-path"
...
export function createToolExecuteAfterHandler(input: {
  ...
  return async (toolInput, toolOutput): Promise<void> => {
    ...
    if (isWriteOrEditToolName(toolInput.tool)) {
      ...
      if (filePath && !isChiefPath(filePath)) {
    ...
    const workstateState = readWorkStateState(ctx.directory)
    ...
    if (toolOutput.output && typeof toolOutput.output === "string") {
      const worktreePath = workstateState?.worktree_path?.trim()
      ...
      if (workstateState) {
        ...
        } = resolveTaskContext(pendingTaskRef, workstateState.active_plan)
        ...
        const lineageSessionIDs = workstateState.session_ids
        ...
        const shouldPauseForApproval = sessionState
          ? shouldPauseForFinalWaveApproval({
              planPath: workstateState.active_plan,
        ...
        const leadReminder = shouldPauseForApproval
          ? buildFinalWaveApprovalReminder(workstateState.plan_name, progress, preferredSessionId)
          : buildCompletionGate(workstateState.plan_name, preferredSessionId)
        const followupReminder = shouldPauseForApproval
          ? null
          : buildOrchestratorReminder(workstateState.plan_name, progress, preferredSessionId, autoCommit, false)
...
${
  followupReminder === null
    ...
    : `<system-reminder>\n${followupReminder}\n</system-reminder>`
        ...
        log(`[${HOOK_NAME}] Output transformed for orchestrator mode (workstate)`, {
