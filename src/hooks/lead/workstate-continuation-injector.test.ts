...
describe("injectWorkStateContinuation", () => {
  beforeEach(() => {
    // given
    _resetForTesting()
  })
  ...
  test("#given background tasks are running #when injector checks again #then it reports skipped background tasks without mutating failure count", async () => {
    ...
    const result = await injectWorkStateContinuation({
      ...
      } as unknown as Parameters<typeof injectWorkStateContinuation>[0]["backgroundManager"],
      sessionState,
    })
  ...
  test("#given recent prompt context includes variant #when injecting workstate continuation #then promptAsync receives variant as a top-level field", async () => {
    // given
    registerAgentName("lead")
