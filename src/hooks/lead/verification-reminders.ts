...
1. **Edit** the plan file \`.cortex/plans/${planName}.md\`:
   - Change \`- [ ]\` to \`- [x]\` for the completed task
   - Use \`Edit\` tool to modify the checkbox

2. **Read** the plan file AGAIN:
   \`\`\`
   Read(".cortex/plans/${planName}.md")
   \`\`\`
...
**WORKSTATE STATE:** Plan: \`${planName}\` | ${progress.completed}/${progress.total} done | ${remaining} remaining
...
\`\`\`
Glob(".cortex/notepads/${planName}/*.md")
\`\`\`
...
**STEP 6: CHECK WORKSTATE STATE DIRECTLY (EVERY TIME - NO EXCEPTIONS)**
...
**${remaining} tasks remain. Keep workstateing.**`
}
...
1. Read \
\`.cortex/plans/${planName}.md\` again and confirm every remaining unchecked **top-level** task belongs to F1-F4.
