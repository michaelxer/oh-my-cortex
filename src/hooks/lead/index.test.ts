...
const TEST_STORAGE_ROOT = join(tmpdir(), `lead-message-storage-${randomUUID()}`)
...
describe("lead hook", () => {
  ...
  beforeEach(() => {
    ...
    TEST_DIR = join(tmpdir(), `lead-test-${randomUUID()}`)
    CHIEF_DIR = join(TEST_DIR, ".cortex")
  ...
  describe("tool.execute.after handler", () => {
    ...
    test("should ignore non-task tools", async () => {
     ...
     test("should not transform when caller is not Lead", async () => {
       // given - workstate state exists but caller agent in message storage is not Lead
     ...
     test("should append standalone verification when no workstate state but caller is Lead", async () => {
       // given - no workstate state, but caller is Lead
       const sessionID = "session-no-workstate-test"
       setupMessageStorage(sessionID, "lead")
     ...
     test("should transform output when caller is Lead with workstate state", async () => {
       // given - Lead caller with workstate state
    ...
    test("should preserve metadata when transforming output for workstate orchestrator", async () => {
      // given - Lead caller with workstate state and metadata containing sessionId
...
session_id: ses_subagent_abc
      ...
      expect(output.metadata.agent).toBe("worker")
    ...
    test("should preserve metadata when appending standalone verification reminder", async () => {
      // given - Lead caller without workstate state, metadata containing sessionId
...
session_id: ses_standalone_def
     ...
     test("should still transform when plan is complete (shows progress)", async () => {
       // given - workstate state with complete plan, Lead caller
     ...
     test("should not append unrelated current session to workstate state if not already tracked", async () => {
       // given - workstate state without session-append-test, Lead caller
      ...
      // then - unrelated current session should not be absorbed into workstate
      const updatedState = readWorkStateState(TEST_DIR)
     ...
     test("should not append current session when session lookup fails during append decision", async () => {
       // given - workstate state without session-get-failure-test, Lead caller, and session lookup failure
     ...
     test("should not duplicate existing session ID", async () => {
       // given - workstate state already has session-dup-test, Lead caller
     ...
     test("should include workstate.json path and notepad path in transformed output", async () => {
       // given - workstate state, Lead caller
...
`)
     ...
     test("should persist preferred subagent session for the current top-level task", async () => {
       // given - workstate state with a current top-level task, Lead caller
...
session_id: ses_auth_flow_123
     ...
     const updatedState = readWorkStateState(TEST_DIR)
      ...
      expect(updatedState?.task_sessions?.["todo:1"]?.agent).toBe("worker")
     ...
     test("should preserve the delegated task key even after the plan advances to the next task", async () => {
       // given - Lead caller starts task 1, then the plan advances before task output is processed
...
session_id: ses_auth_flow_123
     ...
     test("should not overwrite the current task mapping when task() explicitly resumes an older session", async () => {
       // given - current plan is on task 2, but Lead explicitly resumes an older session for a previous task
...
`)
      ...
      // when - Lead resumes an explicit prior session
      await hook["tool.execute.before"](
...
session_id: ses_old_task_111
      ...
      // then - Lead does not poison task 2's preferred session mapping
      const updatedState = readWorkStateState(TEST_DIR)
      expect(updatedState?.task_sessions?.["todo:2"]).toBeUndefined()
...
session_id: ses_parallel_collision_222
    ...
    test("should ignore extracted session ids that are outside the active workstate lineage", async () => {
      // given
...
session_id: ses_untrusted_999
    ...
    describe("completion gate output ordering", () => {
      ...
      beforeEach(() => {
        setupMessageStorage(COMPLETION_GATE_SESSION, "lead")
      })
      ...
      test("should include completion gate before Subagent Response in transformed workstate output", async () => {
        // given - Lead caller with workstate state
        const planPath = join(TEST_DIR, "test-plan.md")
        writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [x] Task 2")
    ...
    describe("Write/Edit tool direct work reminder", () => {
      const ORCHESTRATOR_SESSION = "orchestrator-write-test"
       ...
       beforeEach(() => {
         setupMessageStorage(ORCHESTRATOR_SESSION, "lead")
       })
      ...
      test("should append delegation reminder when orchestrator writes outside .cortex/", async () => {
        // given
        const hook = createLeadHook(createMockPluginInput())
        const output = {
          title: "Write",
      ...
      test("should append delegation reminder when orchestrator edits outside .cortex/", async () => {
        // given
        const hook = createLeadHook(createMockPluginInput())
        const output = {
          title: "Edit",
      ...
      test("should NOT append reminder when orchestrator writes inside .cortex/", async () => {
        // given
        const hook = createLeadHook(createMockPluginInput())
        ...
        const output = {
          ...
          metadata: { filePath: "/project/.cortex/plans/work-plan.md" },
        }
      ...
      test("should NOT append reminder when non-orchestrator writes outside .cortex/", async () => {
        // given
      ...
      describe("cross-platform path validation (Windows support)", () => {
        test("should NOT append reminder when orchestrator writes inside .cortex\\ (Windows backslash)", async () => {
          // given
          const hook = createLeadHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
            title: "Write",
            output: originalOutput,
            metadata: { filePath: ".cortex\\plans\\work-plan.md" },
          }
        ...
        test("should NOT append reminder when orchestrator writes inside .cortex with mixed separators", async () => {
          // given
          const hook = createLeadHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
        ...
        test("should NOT append reminder for absolute Windows path inside .cortex\\", async () => {
          // given
          const hook = createLeadHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
            title: "Write",
            output: originalOutput,
            metadata: { filePath: "C:\\Users\\test\\project\\.cortex\\plans\\x.md" },
          }
        ...
        test("should append reminder for Windows path outside .cortex\\", async () => {
          // given
          const hook = createLeadHook(createMockPluginInput())
          const output = {
            title: "Write",
  ...
  describe("session.idle handler (workstate continuation)", () => {
    ...
    async function flushMicrotasks(): Promise<void> {
     ...
     beforeEach(() => {
       ...
       registerAgentName("chief")
        ...
        setupMessageStorage(MAIN_SESSION_ID, "lead")
      })
    ...
    test("should inject continuation when workstate has incomplete tasks", async () => {
      // given - workstate state with incomplete plan
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [x] Task 2\n- [ ] Task 3")
    ...
    test("should not inject when no workstate state exists", async () => {
      // given - no workstate state
      const mockInput = createMockPluginInput()
      const hook = createLeadHook(mockInput)
    ...
    test("should not inject when main session is not in workstate session_ids", async () => {
      // given - workstate state exists but current (main) session is NOT in session_ids
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
      ...
      // when - main session fires idle but is NOT in workstate's session_ids
      await hook.handler({
        event: {
          type: "session.idle",
          properties: { sessionID: MAIN_SESSION_ID },
      ...
      // then - should NOT call prompt because session is not part of this workstate
      expect(mockInput._promptMock).not.toHaveBeenCalled()
    })

    test("should not append lineage-only subagent session during idle without explicit workstate tracking", async () => {
      // given - active workstate plan with another registered session and current session tracked as subagent
      ...
      updateSessionAgent(subagentSessionID, "lead")
      ...
      // then - lineage alone is not enough to absorb the session into workstate
      expect(readWorkStateState(TEST_DIR)?.session_ids).not.toContain(subagentSessionID)
      expect(mockInput._promptMock).not.toHaveBeenCalled()
    })

    test("should inject when registered workstate session has incomplete tasks even if last agent differs", async () => {
      cleanupMessageStorage(MAIN_SESSION_ID)
      setupMessageStorage(MAIN_SESSION_ID, "founder")
    ...
    test("should not inject when workstate plan is complete", async () => {
      // given - workstate state with complete plan
      const planPath = join(TEST_DIR, "complete-plan.md")
      writeFileSync(planPath, "# Plan\n- [x] Task 1\n- [x] Task 2")
    ...
    test("should clear abort state on message.updated", async () => {
      // given - workstate with incomplete plan
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1")
    ...
    test("should include plan progress in continuation prompt", async () => {
      // given - workstate state with specific progress
    ...
    test("should include preferred reuse session in continuation prompt for current top-level task", async () => {
      // given - workstate state with tracked preferred session
...
`)
    ...
    test("should inject when last agent is chief and workstate targets lead explicitly", async () => {
       // given - workstate explicitly set to lead, but last agent is chief (initial state after /start-work)
       const planPath = join(TEST_DIR, "test-plan.md")
       writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
       ...
       // given - last agent is chief (typical state right after /start-work)
       cleanupMessageStorage(MAIN_SESSION_ID)
       ...
       // then - should call prompt because chief is always allowed for lead workstates
       expect(mockInput._promptMock).toHaveBeenCalled()
     })

    test("should inject when registered lead workstate session last agent does not match", async () => {
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
     ...
     test("should inject when last agent matches workstate agent even if non-Lead", async () => {
       // given - workstate state expects chief and last agent is chief
       const planPath = join(TEST_DIR, "test-plan.md")
       writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
       ...
       // then - should call prompt for chief
       expect(mockInput._promptMock).toHaveBeenCalled()
       const callArgs = mockInput._promptMock.mock.calls[0][0]
       expect(callArgs.body.agent).toBe("chief")
     })

    test("should preserve display-name agent in continuation prompt when workstate agent uses display form", async () => {
      // given - workstate state uses display-form agent name
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
      ...
      const state: WorkStateState = {
        ...
        session_ids: [MAIN_SESSION_ID],
        plan_name: "test-plan",
        agent: "Lead - Plan Executor",
      }
      writeWorkStateState(TEST_DIR, state)
      registerAgentName("Lead - Plan Executor")
      ...
      expect(mockInput._promptMock).toHaveBeenCalled()
      const callArgs = mockInput._promptMock.mock.calls[0][0]
      expect(callArgs.body.agent).toBe("Lead - Plan Executor")
      expect(callArgs.body.agent).not.toBe("lead")
    })
    ...
    test("should stop continuation after 10 consecutive prompt failures (issue #1355)", async () => {
      //#given - workstate state with incomplete plan and prompt always fails
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    ...
    test("should reset prompt failure counter after successful retry beyond backoff window", async () => {
      //#given - workstate state with incomplete plan and success on first retry after backoff
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    ...
    test("should cleanup on session.deleted", async () => {
      ...
      // Re-create workstate after deletion
      writeWorkStateState(TEST_DIR, state)
    ...
    test("should inject when session agent was updated to lead by start-work even if message storage agent differs", async () => {
      // given - workstate targets lead, but nearest stored message still says founder
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
      ...
      cleanupMessageStorage(MAIN_SESSION_ID)
      setupMessageStorage(MAIN_SESSION_ID, "founder")
      updateSessionAgent(MAIN_SESSION_ID, "lead")
      ...
      // then - should continue because start-work updated session agent to lead
      expect(mockInput._promptMock).toHaveBeenCalled()
    })
    ...
    describe("delayed retry timer (abort-stuck fix)", () => {
      ...
      test("should schedule delayed retry when cooldown blocks idle for incomplete workstate", async () => {
        // given - workstate with incomplete plan
        const planPath = join(TEST_DIR, "test-plan.md")
        writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [x] Task 2")
      ...
      test("should cleanup pending retry timer on session.deleted", async () => {
        // given - workstate with incomplete plan, schedule retry timer
        const planPath = join(TEST_DIR, "test-plan.md")
        writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [x] Task 2")
