...
const TEST_STORAGE_ROOT = join(tmpdir(), `lead-final-wave-regression-storage-${randomUUID()}`)
...
describe("Lead final-wave approval gate regressions", () => {
  let testDirectory = ""

  function createMockPluginInput(): LeadHookContext {
    ...
    Reflect.set(client.session, "get", async ({ path }: { path: { id: string } }) => {
      const parentID = path.id === "ses_nested_scope_review"
        ? "lead-nested-final-wave-session"
        : path.id.startsWith("ses_parallel_review_")
          ? "lead-parallel-final-wave-session"
          : "main-session-123"
  ...
  beforeEach(() => {
    testDirectory = join(tmpdir(), `lead-final-wave-regression-${randomUUID()}`)
  ...
  test("waits for approval when nested plan checkboxes remain but the only pending top-level task is final-wave", async () => {
    // given
    const sessionID = "lead-nested-final-wave-session"
...
session_id: ses_nested_scope_review
  ...
  test("waits for approval after the final parallel reviewer approves before plan checkboxes are updated", async () => {
    // given
    const sessionID = "lead-parallel-final-wave-session"
    setupMessageStorage(sessionID)
