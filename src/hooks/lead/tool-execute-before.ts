...
import { readWorkStateState, readCurrentTopLevelTask } from "../../features/work-state"
...
export function createToolExecuteBeforeHandler(input: {
  ...
  return async (toolInput, toolOutput): Promise<void> => {
    ...
    // Warn-only policy: Lead guides orchestrators toward delegation but doesn't block, allowing flexibility for urgent fixes
    ...
    if (toolInput.tool === "task") {
      if (toolInput.callID) {
        ...
        if (requestedSessionId) {
          ...
          const currentTask = workstateState
            ? readCurrentTopLevelTask(workstateState.active_plan)
            : null
          if (currentTask) {
            const task = {
              key: currentTask.key,
