...
import { appendSessionId, type WorkStateState, upsertTaskSessionState } from "../../features/work-state"
...
export async function syncBackgroundLaunchSessionTracking(input: {
  ctx: PluginInput
  workstateState: WorkStateState | null
  ...
  const { ctx, workstateState, toolInput, toolOutput, pendingTaskRef, metadataSessionId } = input
  if (!workstateState) {
    return
  }
