...
import { readWorkStateState, writeWorkStateState } from "../../features/work-state"
...
describe("createToolExecuteAfterHandler background launch detection", () => {
  ...
  beforeEach(() => {
    testDirectory = join(tmpdir(), `lead-background-launch-${crypto.randomUUID()}`)
  ...
  describe("#given a call_cortex_agent background launch result", () => {
    ...
    describe("#when a background task launch belongs to the active workstate task", () => {
...
`)
        ...
        expect(collectGitDiffStatsMock).not.toHaveBeenCalled()
        expect(readWorkStateState(testDirectory)?.session_ids).toContain(childSessionID)
        expect(readWorkStateState(testDirectory)?.session_origins?.[childSessionID]).toBe("appended")
        expect(readWorkStateState(testDirectory)?.task_sessions?.["todo:1"]?.session_id).toBe(childSessionID)
      })
...
`)
        ...
        expect(readWorkStateState(testDirectory)?.session_ids).not.toContain(childSessionID)
      })
...
`)
      ...
      it("#then it should not append an unrelated launcher session into active workstate", async () => {
...
`)
        ...
        writeWorkStateState(testDirectory, {
          active_plan: planPath,
          started_at: "2026-01-02T10:00:00Z",
          session_ids: ["ses_workstate_root"],
          session_origins: { "ses_workstate_root": "direct" },
          plan_name: "background-launch-plan",
        })
        ...
        expect(readWorkStateState(testDirectory)?.session_ids).not.toContain(sessionID)
        expect(readWorkStateState(testDirectory)?.session_ids).not.toContain(childSessionID)
      })
    })
  })
