...
import { createCortexLoopEventHandler } from "./cortex-loop-event-handler"

export interface CortexLoopHook {
...
export function createCortexLoopHook(
  ctx: PluginInput,
  options?: CortexLoopOptions
): CortexLoopHook {
  ...
  const backgroundManager = options?.backgroundManager
	...
	const event = createCortexLoopEventHandler(ctx, {
		directory: ctx.directory,
	...
	return {
		...
		getState: loopState.getState as () => CortexLoopState | null,
	}
}
