...
describe("cortex-loop non-abort error continuation", () => {
	const testDirectory = join(tmpdir(), `cortex-loop-non-abort-error-${Date.now()}`)
	...
	test("continues on next idle after non-abort session error", async () => {
		// given - an active Cortex Loop receives a recoverable command error
