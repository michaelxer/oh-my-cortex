...
import { parseCortexLoopArguments } from "./command-arguments"

describe("cortex-loop", () => {
  const TEST_DIR = join(tmpdir(), "cortex-loop-test-" + Date.now())
  ...
  function createMockPluginInput(): Parameters<typeof createCortexLoopHook>[0] {
    return {
      client: {
        session: {
  ...
  describe("storage", () => {
    ...
    test("should handle deepwork field", () => {
      // given - a state object with deepwork enabled
      const state: CortexLoopState = {
        active: true,
        iteration: 1,
        max_iterations: 50,
      ...
      // then - deepwork field should be preserved
      expect(readResult?.deepwork).toBe(true)
    })
  ...
  describe("command arguments", () => {
    test("should parse --strategy=reset flag", () => {
      // given - cortex-loop command arguments with reset strategy
      const rawArguments = '"Build feature X" --strategy=reset --max-iterations=12'

      // when - parse command arguments
      const parsedArguments = parseCortexLoopArguments(rawArguments)
    ...
    test("should parse --strategy=continue flag", () => {
      // given - cortex-loop command arguments with continue strategy
  ...
  describe("hook", () => {
    ...
    test("should accept deepwork option in startLoop", () => {
      // given - hook instance
      const hook = createCortexLoopHook(createMockPluginInput())

      // when - start loop with deepwork
      hook.startLoop("session-123", "Build something", { deepwork: true })

      // then - state should have deepwork=true
      const state = hook.getState()
      expect(state?.deepwork).toBe(true)
    })

    test("should handle missing deepwork option in startLoop", () => {
      // given - hook instance
      const hook = createCortexLoopHook(createMockPluginInput())

      // when - start loop without deepwork
      hook.startLoop("session-123", "Build something")

      // then - state should have deepwork=undefined
      const state = hook.getState()
      expect(state?.deepwork).toBeUndefined()
    })
    ...
    test("should inject continuation when loop active and no completion detected", async () => {
      ...
      expect(promptCalls[0].text).toContain("CORTEX LOOP")
    ...
    test("should stop loop when max iterations reached", async () => {
      ...
      expect(toastCalls[0].title).toBe("Cortex Loop Stopped")
    ...
    test("should detect completion promise and stop loop", async () => {
      ...
      expect(promptCalls.length).toBe(0)
      expect(toastCalls.some((t) => t.title === "Cortex Loop Complete!")).toBe(true)
      expect(hook.getState()).toBeNull()
    })
    ...
    test("should ignore completion promise in reasoning part via session messages API", async () => {
      ...
      expect(promptCalls.length).toBe(1)
      expect(toastCalls.some((t) => t.title === "Cortex Loop Complete!")).toBe(false)
    ...
    test("should skip concurrent idle events for same session when handler is in flight", async () => {
      ...
      const hook = createCortexLoopHook(mockInput as Parameters<typeof createCortexLoopHook>[0])
    ...
    test("should NOT detect completion from user message in transcript (issue #622)", async () => {
      ...
      // This reproduces the bug where the CORTEX_LOOP_TEMPLATE instructional text
      // containing `<promise>DONE</promise>` is recorded as a user message and
      // falsely triggers completion detection
      const transcriptPath = join(TEST_DIR, "transcript.jsonl")
      const templateText = `You are starting a Cortex Loop...
Output <promise>DONE</promise> when fully complete`
    ...
    test("should NOT detect completion from continuation prompt in transcript (issue #622)", async () => {
      ...
      const transcriptPath = join(TEST_DIR, "transcript.jsonl")
      const continuationText = `CORTEX LOOP 2/100
...
Original task: Build something`
    ...
    test("should require thinker verification toast for deepwork completion promise", async () => {
      // given - hook with deepwork mode and completion in transcript
      const transcriptPath = join(TEST_DIR, "transcript.jsonl")
      const hook = createCortexLoopHook(createMockPluginInput(), {
        getTranscriptPath: () => transcriptPath,
      })
      writeFileSync(transcriptPath, JSON.stringify({ type: "assistant", content: "<promise>DONE</promise>" }) + "\n")
      hook.startLoop("test-id", "Build API", { deepwork: true })

      // when - idle event triggered
      await hook.event({ event: { type: "session.idle", properties: { sessionID: "test-id" } } })

      const verificationToast = toastCalls.find(t => t.title === "DEEPWORK LOOP")
      expect(verificationToast).toBeDefined()
      expect(verificationToast!.message).toMatch(/Thinker verification is now required/)
    })

    test("should show regular completion toast when deepwork disabled", async () => {
      // given - hook without deepwork
      const transcriptPath = join(TEST_DIR, "transcript.jsonl")
      const hook = createCortexLoopHook(createMockPluginInput(), {
        getTranscriptPath: () => transcriptPath,
      })
    ...
    test("should prepend deepwork to continuation prompt when deepwork=true", async () => {
      // given - hook with deepwork mode enabled
      const hook = createCortexLoopHook(createMockPluginInput())
      ...
      // then - prompt should start with "deepwork "
      expect(promptCalls.length).toBe(1)
      expect(promptCalls[0].text).toMatch(/^deepwork /)
    })

    test("should NOT prepend deepwork to continuation prompt when deepwork=false", async () => {
      // given - hook without deepwork mode
      const hook = createCortexLoopHook(createMockPluginInput())
      ...
      // then - prompt should NOT start with "deepwork "
      expect(promptCalls.length).toBe(1)
      expect(promptCalls[0].text).not.toMatch(/^deepwork /)
    })
  })
  ...
  describe("API timeout protection", () => {
    test("should not hang when session.messages() throws", async () => {
      ...
      const hook = createCortexLoopHook(errorMock, {
        getTranscriptPath: () => join(TEST_DIR, "nonexistent.jsonl"),
        apiTimeout: 100,
      })
      hook.startLoop("session-123", "Build something")
