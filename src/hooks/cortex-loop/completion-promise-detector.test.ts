...
describe("detectCompletionInSessionMessages", () => {
  ...
  describe("#given promise appears in tool_result part (not text part)", () => {
    test("#when Thinker returns VERIFIED via task() tool_result #then should detect completion", async () => {
      const messages = [
        {
          info: { role: "assistant" },
          parts: [
            { type: "text", text: "Consulting Thinker for verification." },
            { type: "tool_use", text: '{"subagent_type":"thinker"}' },
          ],
        },
        {
          info: { role: "assistant" },
          parts: [
            ...
            { type: "text", text: "Thinker verified the task." },
          ],
        },
      ]
      const ctx = createPluginInput(messages)
    ...
    test("#when non-Thinker tool_result returns VERIFIED #then should NOT detect completion", async () => {
      const messages = [
        {
          info: { role: "assistant" },
          parts: [
            { type: "tool_result", text: "Agent: tracker\n\n<promise>VERIFIED</promise>" },
            { type: "text", text: "Tracker finished checking." },
          ],
        },
      ]
      const ctx = createPluginInput(messages)
      ...
      const messages = [
        {
          ...
          parts: [
            ...
            { type: "text", text: "Calling Thinker." },
          ],
        },
      ]
      const ctx = createPluginInput(messages)
