...
describe("detectCompletionInSessionMessages negative cases", () => {
	...
	describe("#given promise appears outside assistant text parts", () => {
		test("#when VERIFIED appears only in non-thinker tool_result part #then should NOT detect completion", async () => {
			// #given -- thinker tool_result VERIFIED is detectable (56f2a9df); non-thinker is not
			const messages = [
				{
					info: { role: "assistant" },
					parts: [
						{ type: "tool_result", text: 'Task completed.\n\nAgent: founder\n\n<promise>VERIFIED</promise>' },
						{ type: "text", text: "Founder completed the task." },
					],
				},
			]
			const ctx = createPluginInput(messages)
