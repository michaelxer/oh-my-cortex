...
type CortexLoopEventHandlerOptions = { directory: string; apiTimeoutMs: number; getTranscriptPath: (sessionID: string) => string | undefined; checkSessionExists?: CortexLoopOptions["checkSessionExists"]; backgroundManager?: CortexLoopOptions["backgroundManager"]; loopState: LoopStateController }

export function createCortexLoopEventHandler(
	ctx: PluginInput,
	options: CortexLoopEventHandlerOptions,
) {
			...
			try {
				...
				if (state.verification_pending) {
					if (!verificationSessionID && matchesParentSession) {
						log(`[${HOOK_NAME}] Verification pending without tracked thinker session, running recovery check`, {
							sessionID,
							iteration: state.iteration,
						})
					}
				...
				if (
					...
					await ctx.client.tui?.showToast?.({
						body: { title: "Cortex Loop Stopped", message: `Max iterations (${state.max_iterations}) reached without completion`, variant: "warning", duration: 5000 },
						}).catch(() => {})
					return
				}
