...
import { extractThinkerSessionID, isThinkerVerified } from "./thinker-verification-detector"
...
async function detectThinkerVerificationFromParentSession(
	ctx: PluginInput,
	...
	try {
		...
		for (let index = messageArray.length - 1; index >= 0; index -= 1) {
			...
			if (!isThinkerVerified(assistantText)) {
				continue
			}

			const detectedThinkerSessionID = extractThinkerSessionID(assistantText)
			if (detectedThinkerSessionID) {
				return detectedThinkerSessionID
			}
		}

		return undefined
	} catch (error) {
		log(`[${HOOK_NAME}] Failed to scan parent session for thinker verification evidence`, {
			parentSessionID,
			error: String(error),
		})
		return undefined
...
export async function handlePendingVerification(
	...
	if (matchesParentSession || (verificationSessionID && matchesVerificationSession)) {
		if (!verificationSessionID && state.session_id) {
			const recoveredVerificationSessionID = await detectThinkerVerificationFromParentSession(
				ctx,
				state.session_id,
				directory,
				apiTimeoutMs,
	...
	log(`[${HOOK_NAME}] Waiting for thinker verification`, {
		sessionID,
		verificationSessionID,
