 ...
 * - Windows backslashes (e.g., .cortex\\plans\\x.md)
 * - Mixed separators (e.g., .cortex\\plans/x.md)
 * - Case-insensitive directory/extension matching
 * - Workspace confinement (blocks paths outside root or via traversal)
 * - Nested project paths (e.g., parent/.cortex/... when ctx.directory is parent)
 */
export function isAllowedFile(filePath: string, workspaceRoot: string): boolean {
  ...
  // 4. Check if .cortex/ or .cortex\ exists anywhere in the path (case-insensitive)
  // This handles both direct paths (.cortex/x.md) and nested paths (project/.cortex/x.md)
  if (!/\.cortex[/\\]/i.test(rel)) {
    return false
  }
