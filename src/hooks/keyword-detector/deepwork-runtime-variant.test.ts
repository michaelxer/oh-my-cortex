...
describe("keyword-detector deepwork runtime variant gating", () => {
  test("#given runtime max variant #when deepwork activates #then maximum precision toast is preserved", async () => {
    // given
    _resetForTesting()
    ...
    const output = {
      ...
      parts: [{ type: "text", text: "deepwork do it" }],
    }
  ...
  test("#given runtime non-max variant #when deepwork activates #then variant stays unchanged and toast does not claim max", async () => {
    // given
    _resetForTesting()
    setMainSession("main-session")
    const toastMessages: string[] = []
