/**
 * Default deepwork message optimized for Claude series models.
 *
 * Key characteristics:
 * - Natural tool-like usage of tracker/researcher agents (run_in_background=true)
 * - Parallel execution emphasized - fire agents and continue working
 * - Simple workflow: EXPLORES → GATHER → PLAN → DELEGATE
 */

export const DEEPWORK_DEFAULT_MESSAGE = `<deepwork-mode>
...
| **TRACKER** the codebase to understand existing patterns, architecture, and context |
...
2. **TRACKER THOROUGHLY** - Fire tracker/researcher agents to gather ALL relevant context
...
task(subagent_type="thinker", load_skills=[], prompt="I need architectural review of my approach to [TASK]. Here's my plan: [DESCRIBE PLAN WITH SPECIFIC FILES AND CHANGES]. My concerns are: [LIST SPECIFIC UNCERTAINTIES]. Please evaluate: correctness of approach, potential issues I'm missing, and whether a better alternative exists.", run_in_background=false)
\`\`\`
...
3. **DO** consult specialists (thinker for conventional, artistry for non-conventional)
4. **DO** ask the user for guidance
5. **DO** tracker alternative approaches
...
|-----------|--------|-----|
| Codebase exploration | task(subagent_type="tracker", load_skills=[], run_in_background=true) | Parallel, context-efficient |
...
| Hard problem (conventional) | task(subagent_type="thinker", load_skills=[], run_in_background=false) | Architecture, debugging, complex logic |
...
export function getDefaultDeepworkMessage(): string {
  return DEEPWORK_DEFAULT_MESSAGE
}
