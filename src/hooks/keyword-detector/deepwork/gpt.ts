/**
 * Deepwork message optimized for GPT 5.4 series models.
 *
 * Design principles:
...
export const DEEPWORK_GPT_MESSAGE = `<deepwork-mode>
...
- If the question is ambiguous or underspecified:
  - TRACKER FIRST using tools (grep, file reads, tracker agents)
...
| **Research** | Need broad codebase context or external docs | **DELEGATE** to tracker/researcher (background, parallel) |
...
|----------|-------------|------------|
| tracker agent | Need codebase patterns you don't have | \`task(subagent_type="tracker", load_skills=[], run_in_background=true, ...)\` |
| researcher agent | External library docs, OSS examples | \`task(subagent_type="researcher", load_skills=[], run_in_background=true, ...)\` |
| thinker agent | Stuck on architecture/debugging after 2+ attempts | \`task(subagent_type="thinker", load_skills=[], run_in_background=false, ...)\` |
...
- Parallelize independent reads (read_file, grep, tracker, researcher) to reduce latency
...
| **Background** | tracker, researcher agents | Async | Deep search, external docs |
...
export function getGptDeepworkMessage(): string {
  return DEEPWORK_GPT_MESSAGE;
}
