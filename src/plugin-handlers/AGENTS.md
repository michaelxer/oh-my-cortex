...
The canonical agent order is **chief → founder → planner → lead**.
...
- Removing the prefix and relying on insertion order alone falls back to alphabetical Lead → Founder → Planner → Chief.
...
- #3260 (merged): removed ZWSP injection. Reverted by `0d5b08744` because OpenCode 1.4.x ignores `order`, and removal alone causes alphabetical fallback (Lead → Founder → Planner → Chief).
...
| `planner-agent-config-builder.ts` | ~100 | Planner config with model resolution |
| `plan-model-inheritance.ts` | 28 | Plan demotion logic |
| `agent-priority-order.ts` | ~30 | chief, founder, planner, lead first |
...
|-------|---------|--------|
| Researcher | grep_app_* | — |
| Lead, Chief, Planner | task, task_*, teammate | — |
| Founder | task | — |
...
```
User (~/.config/opencode/oh-my-cortex.jsonc)
  ↓ deepMerge
Project (.opencode/oh-my-cortex.jsonc)
  ↓ Zod defaults
Final Config
```
