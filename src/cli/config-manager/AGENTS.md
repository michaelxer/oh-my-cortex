...
| File | Purpose |
|------|---------|
| `add-plugin-to-opencode-config.ts` | Register `oh-my-cortex` in `.opencode/opencode.json` plugin array |
...
| `generate-omx-config.ts` | Generate `oh-my-cortex.jsonc` from install selections |
...
| `plugin-name-with-version.ts` | Resolve `oh-my-cortex@X.Y.Z` for installation |
| `write-omx-config.ts` | Write generated config to `.opencode/oh-my-cortex.jsonc` |
...
5. generate-omx-config (build config from user selections)
6. write-omx-config
