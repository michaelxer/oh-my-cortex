...
import { writeOmxConfig } from "./write-omx-config"
...
describe("writeOmxConfig", () => {
  let testConfigDir = ""
  let testConfigPath = ""

  beforeEach(() => {
    testConfigDir = join(tmpdir(), `omx-write-config-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  it("preserves existing user values while adding new defaults", () => {
    ...
    const generatedDefaults = generateOmxConfig(installConfig)

    // when
    const result = writeOmxConfig(installConfig)
    ...
    const savedChief = getRecord(savedAgents.cortex)
    expect(savedChief.model).toBe("custom/provider-model")
