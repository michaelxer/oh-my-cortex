...
program
  .command("install")
  .description("Install and configure oh-my-cortex with interactive setup")
...
Model Providers (Priority: Native > Copilot > OpenCode Zen > Z.ai > Kimi > Vercel):
  ...
  OpenAI        Native openai/ models (GPT-5.4 for Thinker)
  Gemini        Native google/ models (Gemini 3.1 Pro, Flash)
  Copilot       github-copilot/ models (fallback)
  OpenCode Zen  opencode/ models (opencode/claude-opus-4-7, etc.)
  Z.ai          zai-coding-plan/glm-5 (visual-engineering fallback)
  Kimi          kimi-for-coding/k2p5 (Chief/Planner fallback)
...
program
  ...
  .option("-a, --agent <name>", "Agent to use (default: from CLI/env/config, fallback: Chief)")
  ...
  .addHelpText("after", `
Examples:
  $ bunx oh-my-cortex run "Fix the bug in index.ts"
  $ bunx oh-my-cortex run --agent Chief "Implement feature X"
  $ bunx oh-my-cortex run --port 4321 "Fix the bug"
  $ bunx oh-my-cortex run --attach http://127.0.0.1:4321 "Fix the bug"
  $ bunx oh-my-cortex run --json "Fix the bug" | jq .sessionId
  $ bunx oh-my-cortex run --on-complete "notify-send Done" "Fix the bug"
  $ bunx oh-my-cortex run --session-id ses_abc123 "Continue the work"
  $ bunx oh-my-cortex run --model anthropic/claude-sonnet-4 "Fix the bug"
  $ bunx oh-my-cortex run --agent Chief --model openai/gpt-5.5 "Implement feature X"

Agent resolution order:
  1) --agent flag
  2) OPENCODE_DEFAULT_AGENT
  3) oh-my-cortex.json "default_run_agent"
  4) Chief (fallback)

Available core agents:
  Chief, Founder, Planner, Lead
...
Examples:
  ...
  $ bunx oh-my-cortex get-local-version --json
  $ bunx oh-my-cortex get-local-version --directory /path/to/project
...
program
  .command("doctor")
  .description("Check oh-my-cortex installation health and diagnose issues")
  .option("--status", "Show compact system dashboard")
  .option("--verbose", "Show detailed diagnostic information")
  .option("--json", "Output results in JSON format")
  .addHelpText("after", `
Examples:
  $ bunx oh-my-cortex doctor            # Show problems only
  $ bunx oh-my-cortex doctor --status   # Compact dashboard
  $ bunx oh-my-cortex doctor --verbose  # Deep diagnostics
  $ bunx oh-my-cortex doctor --json     # JSON output
`)
  .action(async (options) => {
...
program
  ...
  .option("-d, --directory <path>", "Working directory to read oh-my-cortex config from")
