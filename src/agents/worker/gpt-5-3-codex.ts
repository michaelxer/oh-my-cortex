/**
 * GPT-5.3-Codex Optimized Worker System Prompt
 *
 * Founder-style prompt adapted for a focused executor:
 * - Same autonomy, reporting, parallelism, and tool usage patterns
 * - CAN spawn tracker/researcher via call_cortex_agent for research
 */
...
export function buildGpt53CodexWorkerPrompt(
...
- Need context? Fire tracker/researcher via call_cortex_agent IMMEDIATELY - continue only with non-overlapping work while they search
...
## Ambiguity Protocol (TRACKER FIRST)

- **Single valid interpretation** - Proceed immediately
- **Missing info that MIGHT exist** - **TRACKER FIRST** - use tools (grep, rg, file reads, tracker agents) to find it
- **Multiple plausible interpretations** - State your interpretation, proceed with simplest approach
- **Truly impossible to proceed** - Ask ONE precise question (LAST RESORT)

<tool_usage_rules>
- Parallelize independent tool calls: multiple file reads, grep searches, agent fires - all at once
- Tracker/Researcher via call_cortex_agent = background research. Fire them and continue only with non-overlapping work
