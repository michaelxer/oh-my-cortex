...
describe("createWorkerAgentWithOverrides", () => {
  ...
  describe("tool safety (task blocked, call_cortex_agent allowed)", () => {
    test("task remains blocked, call_cortex_agent is allowed via tools format", () => {
      // given
      const override = {
        tools: {
          task: true,
      ...
      if (tools) {
        ...
        // call_cortex_agent is NOW ALLOWED for subagents to spawn tracker/researcher
        expect(tools.call_cortex_agent).toBe(true)
        expect(tools.read).toBe(true)
      }
      if (permission) {
        expect(permission.task).toBe("deny")
        // call_cortex_agent is NOW ALLOWED for subagents to spawn tracker/researcher
        expect(permission.call_cortex_agent).toBe("allow")
      }
    })
    ...
    test("task remains blocked when using permission format override", () => {
      ...
      // then - task blocked, but call_cortex_agent allowed for tracker/researcher spawning
      const tools = result.tools as Record<string, boolean> | undefined
      const permission = result.permission as Record<string, string> | undefined
      if (tools) {
        expect(tools.task).toBe(false)
  ...
  describe("useTaskSystem integration", () => {
    ...
    test("useTaskSystem=false uses todowrite instead of task_create", () => {
      ...
      //#when
      const result = createWorkerAgentWithOverrides(override, undefined, false)
  ...
  describe("prompt composition", () => {
    ...
    test("GPT model uses GPT-optimized prompt with Founder-style sections", () => {
      // given
      const override = { model: "openai/gpt-5.4" }
    ...
    test("GPT variants deny apply_patch while Claude variants do not", () => {
      ...
      // when
      const gpt54Result = createWorkerAgentWithOverrides(gpt54Override)
      const gpt53Result = createWorkerAgentWithOverrides(gpt53Override)
      const gptGenericResult = createWorkerAgentWithOverrides(gptGenericOverride)
      const claudeResult = createWorkerAgentWithOverrides(claudeOverride)
