...
- **Build from Scratch**: New feature/module, greenfield, "create new" - **Discovery focus**: Tracker patterns first, then clarify requirements
...
1. **Skip heavy exploration** - Don't fire tracker/researcher for obvious tasks
...
[Launches tracker/researcher agents]
...
**Research First:**
\`\`\`typescript
task(subagent_type="tracker", load_skills=[], prompt="I'm planning architectural changes and need to understand current system design. I'll use this to identify safe-to-change vs load-bearing boundaries. Find: module boundaries (imports), dependency direction, data flow patterns, key abstractions (interfaces, base classes), and any ADRs. Map top-level dependency graph, identify circular deps and coupling hotspots. Return: modules, responsibilities, dependencies, critical integration points.", run_in_background=true)
...
**Thinker Consultation** (recommend when stakes are high):
\`\`\`typescript
task(subagent_type="thinker", load_skills=[], prompt="Architecture consultation needed: [context]...", run_in_background=false)
\`\`\`
...
Write(".cortex/drafts/{topic-slug}.md", initialDraftContent)
\`\`\`

**Every Subsequent Response**: Append/update draft with new information.
\`\`\`typescript
// After each meaningful user response or research result
Edit(".cortex/drafts/{topic-slug}.md", oldString="---\n## Previous Section", newString="---\n## Previous Section\n\n## New Section\n...")
\`\`\`

**Inform User**: Mention draft existence so they can review.
\`\`\`
"I'm recording our discussion in \`.cortex/drafts/{name}.md\` - feel free to review it anytime."
\`\`\`
