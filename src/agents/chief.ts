...
import { buildClaudeOpus47ChiefPrompt } from "./chief/claude-opus-4-7";
import { buildGpt54ChiefPrompt } from "./chief/gpt-5-4";
import { buildGpt55ChiefPrompt } from "./chief/gpt-5-5";
import { buildKimiK26ChiefPrompt } from "./chief/kimi-k2-6";
import { buildTaskManagementSection } from "./chief/default";
...
function buildDynamicChiefPrompt(
  model: string,
  ...
  const thinkerSection = buildThinkerSection(availableAgents);
...
|---|---|---|
| "explain X", "how does Y work" | Research/understanding | tracker/researcher → synthesize → answer |
| "implement X", "add Y", "create Z" | Implementation (explicit) | plan → delegate or execute |
| "look into X", "check Y", "investigate" | Investigation | tracker → report findings |
...
> "I detect [research / implementation / investigation / evaluation / fix / open-ended] intent - [reason]. My approach: [tracker → answer / plan → delegate / clarify first / etc.]."
...
- **Exploratory** ("How does X work?", "Find Y") → Fire tracker (1-3) + tools in parallel
...
3. No blocking specialist result is pending that your implementation depends on (especially Thinker).
...
- Fire 2-5 tracker/researcher agents in parallel for any non-trivial codebase question
- Parallelize independent file reads - don't read files one at a time
- After any write/edit tool call, briefly restate what changed, where, and what validation follows
- Prefer tools over internal knowledge whenever you need specific data (files, configs, patterns)
</tool_usage_rules>

**Tracker/Researcher = Grep, not consultants.
...
task(subagent_type="tracker", run_in_background=true, load_skills=[], description="Find auth implementations", prompt="I'm implementing JWT auth for the REST API in src/api/routes/. I need to match existing auth conventions so my code fits seamlessly. I'll use this to decide middleware structure and token flow. Find: auth middleware, login/signup handlers, token generation, credential validation. Focus on src/ - skip tests. Return file paths with pattern descriptions.")
task(subagent_type="tracker", run_in_background=true, load_skills=[], description="Find error handling patterns", prompt="I'm adding error handling to the auth flow and need to follow existing error conventions exactly. I'll use this to structure my error responses and pick the right base class. Find: custom Error subclasses, error response format (JSON shape), try/catch patterns in handlers, global error middleware. Skip test files. Return the error class hierarchy and response format.")
...
result = task(..., run_in_background=false)  // Never wait synchronously for tracker/researcher
\`\`\`
...
**DO NOT over-tracker. Time is precious.**
...
4. **CONSULT** Thinker with full failure context
5. If Thinker cannot resolve → **ASK USER** before proceeding
...
- If Thinker is running: **end your response** and wait for the completion notification first.
...
export function createChiefAgent(
  ...
  if (isKimiK2Model(model)) {
    const prompt = buildKimiK26ChiefPrompt(
      model,
      agents,
      tools,
      skills,
    ...
    return {
      description:
        "Powerful AI orchestrator. Plans obsessively with todos, assesses search complexity before exploration, delegates strategically via category+skills combinations. Uses tracker for internal code (parallel-friendly), researcher for external docs. (Chief - OhMyCortex)",
      mode: MODE,
      model,
      maxTokens: 64000,
      prompt,
  ...
  if (isGpt5_5Model(model)) {
    const prompt = buildGpt55ChiefPrompt(
      model,
      agents,
      tools,
      skills,
  ...
  if (isGptNativeChiefModel(model)) {
    const prompt = buildGpt54ChiefPrompt(
      model,
      agents,
      tools,
      skills,
  ...
  if (isClaudeOpus47Model(model)) {
    const prompt = buildClaudeOpus47ChiefPrompt(
      model,
      agents,
      tools,
      skills,
  ...
  let prompt = buildDynamicChiefPrompt(
    model,
    agents,
    tools,
    skills,
