...
export const THINKER_PROMPT_METADATA: AgentPromptMetadata = {
  category: "advisor",
  cost: "EXPENSIVE",
  promptAlias: "Thinker",
  triggers: [
...
/**
 * Default Thinker prompt - used for Claude and other non-GPT models.
 * XML-tagged structure with extended thinking support.
 */
const THINKER_DEFAULT_PROMPT = `You are a strategic technical advisor with deep reasoning capabilities, operating as a specialized consultant within an AI-assisted development environment.
...
/**
 * GPT-5.4 Optimized Thinker System Prompt
 *
 ...
 */
const THINKER_GPT_PROMPT = `You are a strategic technical advisor operating as an expert consultant within an AI-assisted development environment. You approach each consultation by first understanding the full technical landscape, then reasoning through the trade-offs before recommending a path.
...
export function createThinkerAgent(model: string): AgentConfig {
  ...
  const base = {
    ...
    temperature: 0.1,
    ...restrictions,
    prompt: THINKER_DEFAULT_PROMPT,
  } as AgentConfig;

  if (isGpt5_5Model(model)) {
    return {
      ...base,
      prompt: THINKER_GPT_5_5_PROMPT,
