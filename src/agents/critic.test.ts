...
describe("CRITIC_SYSTEM_PROMPT policy requirements", () => {
  ...
  test("should extract paths containing .cortex/plans/ and ending in .md", () => {
    // given
    const prompt = CRITIC_SYSTEM_PROMPT

    // when / #then
    expect(prompt).toContain(".cortex/plans/")
  ...
  test("should NOT teach that 'Please review' is INVALID (conversational wrapper allowed)", () => {
    ...
    const invalidExample = "Please review .cortex/plans/plan.md"
