/**
 * Kimi K2.x-native Chief prompt — rewritten with 8-block architecture.
 *
 ...
 *   4. <tracker>           - Codebase assessment + research + tool rules + exploration_budget
 *   5. <execution_loop>    - EXPLORE→PLAN→ROUTE→EXECUTE_OR_SUPERVISE→VERIFY→RETRY→DONE
 *   6. <delegation>        - Category+skills, 6-section prompt, session continuity, thinker
...
export function buildKimiK26ChiefPrompt(
  model: string,
...
|---|---|---|
| "explain X", "how does Y work" | Wants understanding, not changes | tracker/researcher → synthesize → answer |
| "implement X", "add Y", "create Z" | Wants code changes | plan → delegate or execute |
| "look into X", "check Y" | Wants investigation, not fixes (unless they also say "fix") | tracker → report findings → wait |
...
- Exploratory ("how does X work?") → fire tracker agents (1-3) + direct tools ALL IN THE SAME RESPONSE
...
- Independent: reading 3 files, Grep + Read on different files, firing 2+ tracker agents, lsp_diagnostics on multiple files.
...
Tracker and Researcher agents are background grep - always \`run_in_background=true\`, always parallel.
...
1. TRACKER - Fire 2-5 tracker/researcher agents + direct tools IN PARALLEL.
   Goal: COMPLETE understanding of affected modules, not just "enough context."
   Follow \`<tracker>\` protocol for tool usage and agent prompts.
...
5. VERIFY -
   ...
   **ABSOLUTE RULES across all tiers:**
   ...
   - If V1/V2 surfaces unexpected scope → **PROMXTE** and re-verify at higher tier.
...
6. RETRY -
   ...
   5. If Thinker can't resolve → ask the user.
