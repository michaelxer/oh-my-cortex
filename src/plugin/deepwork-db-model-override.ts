...
function retryViaMicrotask(
  ...
  if (attempt >= MAX_MICROTASK_RETRIES) {
    log("[deepwork-db-override] Exhausted microtask retries, falling back to setTimeout", {
      messageId,
      attempt,
    })
    setTimeout(() => {
      try {
        if (tryUpdateMessageModel(db, messageId, targetModel, variant)) {
          log(`[deepwork-db-override] setTimeout fallback succeeded: ${targetModel.providerID}/${targetModel.modelID}`, { messageId })
        } else {
          log("[deepwork-db-override] setTimeout fallback failed - message not found", { messageId })
        }
      } catch (error) {
        log("[deepwork-db-override] setTimeout fallback failed with error", {
          messageId,
          error: String(error),
        })
      } finally {
        try {
          ...
          log("[deepwork-db-override] Failed to close DB after setTimeout fallback", {
            messageId,
            error: String(error),
          })
        }
  ...
  queueMicrotask(() => {
    ...
    try {
      if (tryUpdateMessageModel(db, messageId, targetModel, variant)) {
        log(`[deepwork-db-override] Deferred DB update (attempt ${attempt}): ${targetModel.providerID}/${targetModel.modelID}`, { messageId })
        return
      }
      ...
      log("[deepwork-db-override] Deferred DB update failed with error", {
        messageId,
        attempt,
        error: String(error),
      })
      ...
      if (shouldCloseDb) {
        try {
          ...
          log("[deepwork-db-override] Failed to close DB after deferred DB update", {
            messageId,
            attempt,
            error: String(error),
          })
...
export function scheduleDeferredModelOverride(
  ...
  queueMicrotask(() => {
    ...
    if (!existsSync(dbPath)) {
      log("[deepwork-db-override] DB not found, skipping deferred override")
      return
    }
    ...
    try {
      ...
      log("[deepwork-db-override] Failed to open DB, skipping deferred override", {
        messageId,
        error: String(error),
      })
      return
    ...
    try {
      ...
      log("[deepwork-db-override] Failed to apply deferred model override", {
        error: String(error),
      })
      db.close()
    }
