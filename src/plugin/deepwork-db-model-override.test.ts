...
let scheduleDeferredModelOverride: (typeof import("./deepwork-db-model-override"))["scheduleDeferredModelOverride"]

async function importFreshDeepworkDbModelOverrideModule(): Promise<typeof import("./deepwork-db-model-override")> {
  return import(`./deepwork-db-model-override?test=${Date.now()}-${Math.random()}`)
}
...
describe("scheduleDeferredModelOverride", () => {
  ...
  beforeEach(async () => {
    tempDir = mkdtempSync(join(tmpdir(), "deepwork-db-test-"))
    ...
    ;({ scheduleDeferredModelOverride } = await importFreshDeepworkDbModelOverrideModule())
  })
