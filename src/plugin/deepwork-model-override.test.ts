...
let resolveDeepworkOverride: (typeof import("./deepwork-model-override"))["resolveDeepworkOverride"]
let detectDeepwork: (typeof import("./deepwork-model-override"))["detectDeepwork"]
let applyDeepworkModelOverrideOnMessage: (typeof import("./deepwork-model-override"))["applyDeepworkModelOverrideOnMessage"]

async function importFreshDeepworkModelOverrideModule(): Promise<typeof import("./deepwork-model-override")> {
  return import(`./deepwork-model-override?test=${Date.now()}-${Math.random()}`)
}

async function loadFreshDeepworkModelOverrideModule(): Promise<void> {
  ;({
    resolveDeepworkOverride,
    detectDeepwork,
    applyDeepworkModelOverrideOnMessage,
  } = await importFreshDeepworkModelOverrideModule())
}

describe("detectDeepwork", () => {
  beforeEach(async () => {
    await loadFreshDeepworkModelOverrideModule()
  })

  test("should detect deepwork keyword", () => {
    expect(detectDeepwork("deepwork do something")).toBe(true)
  })

  test("should detect dw keyword", () => {
    expect(detectDeepwork("dw fix the bug")).toBe(true)
  })
  ...
  test("should not detect in code blocks", () => {
    ...
    expect(detectDeepwork(textWithCodeBlock)).toBe(false)
  })

  test("should not detect in inline code", () => {
    expect(detectDeepwork("the `deepwork` mode is cool")).toBe(false)
  })

  test("should not detect when keyword absent", () => {
    expect(detectDeepwork("just do something normal")).toBe(false)
  })
})

describe("resolveDeepworkOverride", () => {
  beforeEach(async () => {
    await loadFreshDeepworkModelOverrideModule()
  })
  ...
  function createConfig(agentName: string, deepwork: { model?: string; variant?: string }) {
    return {
      agents: {
        [agentName]: { deepwork },
      },
    } as unknown as Parameters<typeof resolveDeepworkOverride>[0]
  }

  test("should resolve override when deepwork keyword detected", () => {
    //#given
    const config = createConfig("chief", { model: "anthropic/claude-opus-4-7", variant: "max" })
    const output = createOutput("deepwork do something")

    //#when
    const result = resolveDeepworkOverride(config, "chief", output)
  ...
  test("should return null when no keyword detected", () => {
    //#given
    const config = createConfig("chief", { model: "anthropic/claude-opus-4-7" })
  ...
  test("should return null when agent name is undefined", () => {
    ...
    //#when
    const result = resolveDeepworkOverride(config, undefined, output)
  ...
  test("should use message.agent when input agent is undefined", () => {
    //#given
    const config = createConfig("chief", { model: "anthropic/claude-opus-4-7" })
    const output = createOutput("deepwork do something", "chief")
  ...
  test("should return null when agents config is missing", () => {
    //#given
    const config = {} as Parameters<typeof resolveDeepworkOverride>[0]
    const output = createOutput("deepwork do something")
  ...
  test("should return null when agent has no deepwork config", () => {
    //#given
    const config = {
      agents: { chief: { model: "anthropic/claude-sonnet-4-6" } },
    } as unknown as Parameters<typeof resolveDeepworkOverride>[0]
    const output = createOutput("deepwork do something")
  ...
  test("should resolve variant-only override when deepwork.model is not set", () => {
    //#given
    const config = createConfig("chief", { variant: "max" })
    const output = createOutput("deepwork do something")
  ...
  test("should handle model string with multiple slashes", () => {
    //#given
    const config = createConfig("chief", { model: "openai/gpt-5.3/codex" })
    const output = createOutput("deepwork do something")
  ...
  test("should return null when model string has no slash", () => {
    //#given
    const config = createConfig("chief", { model: "just-a-model" })
    const output = createOutput("deepwork do something")
  ...
  test("should resolve display name to config key", () => {
    //#given
    const config = createConfig("chief", { model: "anthropic/claude-opus-4-7", variant: "max" })
    const output = createOutput("dw do something")

    //#when
    const result = resolveDeepworkOverride(config, "Chief - Deepworker", output)
  ...
  test("should use session agent when input and message agents are undefined", () => {
    ...
    getSessionAgentSpy.mockReturnValue("chief")

    //#when
    const result = resolveDeepworkOverride(config, undefined, output, "ses_test")
...
describe("applyDeepworkModelOverrideOnMessage", () => {
  ...
  function createConfig(agentName: string, deepwork: { model?: string; variant?: string }) {
    ...
    } as unknown as Parameters<typeof applyDeepworkModelOverrideOnMessage>[0]
  }

  test("should schedule deferred DB override without variant when SDK unavailable", () => {
    //#given
    const config = createConfig("chief", { model: "anthropic/claude-opus-4-7", variant: "max" })
    const output = createOutput("deepwork do something", { messageId: "msg_123" })
    const tui = createMockTui()

    //#when - no client passed, SDK validation unavailable
    applyDeepworkModelOverrideOnMessage(config, "chief", output, tui)
  ...
  test("should NOT override variant when SDK unavailable even if config specifies variant", () => {
    //#given
    const config = createConfig("chief", {
  ...
  test("should not apply variant-only override when no SDK available", () => {
    //#given
    const config = createConfig("chief", { variant: "high" })
    const output = createOutput("deepwork do something")
    const tui = createMockTui()
  ...
  test("should log the model transition with deferred DB tag", () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ agent: "chief" }),
    )
  })
  ...
  test("should resolve display name to config key with deferred path", () => {
    //#given
    const config = createConfig("chief", { model: "anthropic/claude-opus-4-7", variant: "max" })
    const output = createOutput("dw do something", { messageId: "msg_123" })
    const tui = createMockTui()

    //#when
    applyDeepworkModelOverrideOnMessage(config, "Chief - Deepworker", output, tui)
  ...
  test("should skip override trigger when current model already matches deepwork model", () => {
    //#given
    const config = createConfig("chief", { model: "anthropic/claude-opus-4-7", variant: "max" })
    const output = createOutput("deepwork do something", {
  ...
  test("should apply validated variant when SDK confirms model supports it", async () => {
    ...
    //#when
    await applyDeepworkModelOverrideOnMessage(config, "chief", output, tui, undefined, mockClient)
  ...
  test("should NOT apply variant when SDK confirms model does NOT have it", async () => {
    //#given
    const config = createConfig("chief", { model: "anthropic/claude-haiku-4-5", variant: "max" })
    const output = createOutput("deepwork do something", { messageId: "msg_123" })
    const tui = createMockTui()
    const mockClient = {
      provider: {
