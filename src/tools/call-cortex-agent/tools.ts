...
import { ALLOWED_AGENTS, CALL_OMX_AGENT_DESCRIPTION } from "./constants"
import type { CallCortexAgentArgs, ToolContextWithMetadata } from "./types"
...
function resolveModelAndFallbackChain(args: {
  ...
  if (agentOverride?.model) {
    ...
    if (normalized) {
      ...
      log("[call_cortex_agent] Resolved model override from agent config", {
        agent: subagentType,
    ...
    if (normalized) {
      ...
      log("[call_cortex_agent] Resolved model override from agent category", {
        agent: subagentType,
...
export function createCallCortexAgent(
  ctx: PluginInput,
  ...
  const description = CALL_OMX_AGENT_DESCRIPTION.replace(
    "{agents}",
    agentDescriptions,
  );
  ...
  return tool({
    ...
    async execute(args: CallCortexAgentArgs, toolContext) {
      const toolCtx = toolContext as ToolContextWithMetadata;
      log(
        `[call_cortex_agent] Starting with agent: ${args.subagent_type}, background: ${args.run_in_background}`,
      );

      const callableAgents = await resolveCallableAgents(ctx.client);

      // Strip ZWSP and case-insensitive agent validation - allows "Tracker", "TRACKER", "tracker" etc.
