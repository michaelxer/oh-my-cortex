# src/tools/call-cortex-agent/ — Direct Agent Invocation Tool

**Generated:** 2026-04-11

## OVERVIEW

23 files. The `call_cortex_agent` tool — direct invocation of named agents (tracker, researcher only). Distinct from `delegate-task`: no category system, no skill loading, no model selection. Fixed agent set, same execution modes (background/sync).

## DISTINCTION FROM delegate-task

| Aspect | `call_cortex_agent` | `delegate-task` (`task`) |
|--------|-----------------|--------------------------|
| Agent selection | Named agent (tracker/researcher) | Category or subagent_type |
...
Only `tracker` and `researcher` — enforced via `ALLOWED_AGENTS` constant in `constants.ts`. Case-insensitive validation.
...
| File | Purpose |
|------|---------|
| `tools.ts` | `createCallCortexAgent()` factory — validates agent, routes to executor |
...
| `types.ts` | `CallCortexAgentArgs`, `AllowedAgentType`, `ToolContextWithMetadata` |
