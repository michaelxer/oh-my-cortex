...
const { createCallCortexAgent } = require("./tools")
...
const DEFAULT_AGENTS = [
  ...
  { name: "founder", mode: "subagent" },
  { name: "reviewer", mode: "subagent" },
  { name: "critic", mode: "subagent" },
  { name: "spotter", mode: "subagent" },
]
...
describe("createCallCortexAgent", () => {
  describe("disabled_agents validation", () => {
    test("should reject agent in disabled_agents list", async () => {
      ...
      const toolDef = createCallCortexAgent(mockCtx, mockBackgroundManager, ["tracker"])
      const executeFunc = toolDef.execute as Function

      const result = await executeFunc(
        { description: "Test", prompt: "Test prompt", subagent_type: "tracker", run_in_background: true },
        toolCtx
      )
    ...
    test("should allow agent not in disabled_agents list", async () => {
      const mockCtx = createMockCtx(DEFAULT_AGENTS)
      const toolDef = createCallCortexAgent(mockCtx, mockBackgroundManager, ["researcher"])
      const executeFunc = toolDef.execute as Function
    ...
    test("should allow all agents when disabled_agents is empty", async () => {
      const mockCtx = createMockCtx(DEFAULT_AGENTS)
      const toolDef = createCallCortexAgent(mockCtx, mockBackgroundManager, [])
      const executeFunc = toolDef.execute as Function
  ...
  describe("dynamic custom agent resolution", () => {
    ...
    test("should exclude primary-mode agents from callable list", async () => {
      ...
      const result = await executeFunc(
        { description: "Test", prompt: "Orchestrate", subagent_type: "chief", run_in_background: true },
        toolCtx
      )
    ...
    test("should fall back to ALLOWED_AGENTS when client.app.agents() fails", async () => {
      ...
      const result = await executeFunc(
        { description: "Test", prompt: "Tracker codebase", subagent_type: "tracker", run_in_background: true },
        toolCtx
      )
    ...
    test("should still apply disabled_agents check to dynamically resolved custom agents", async () => {
      const agents = [...DEFAULT_AGENTS, { name: "bug-fixer", mode: "subagent" }]
      const mockCtx = createMockCtx(agents)
      const toolDef = createCallCortexAgent(mockCtx, mockBackgroundManager, ["bug-fixer"])
      const executeFunc = toolDef.execute as Function
  ...
  test("uses agent override fallback_models when launching background subagent", async () => {
    ...
    const mockCtx = createMockCtx(DEFAULT_AGENTS)
    const toolDef = createCallCortexAgent(
      mockCtx,
      managerWithLaunch,
      [],
      {
