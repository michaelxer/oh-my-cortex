import type { CallCortexAgentArgs } from "./types"
import type { PluginInput } from "@opencode-ai/plugin"
import { subagentSessions, syncSubagentSessions } from "../../features/claude-code-session-state"
import { log } from "../../shared"

export async function createOrGetSession(
  args: CallCortexAgentArgs,
  toolContext: {
    sessionID: string
    messageID: string
    agent: string
  ...
  if (args.session_id) {
    log(`[call_cortex_agent] Using existing session: ${args.session_id}`)
    const sessionResult = await ctx.client.session.get({
      path: { id: args.session_id },
    })
    if (sessionResult.error) {
      log(`[call_cortex_agent] Session get error:`, sessionResult.error)
      throw new Error(`Failed to get existing session: ${sessionResult.error}`)
    }
    return { sessionID: args.session_id, isNew: false }
  } else {
    log(`[call_cortex_agent] Creating new session with parent: ${toolContext.sessionID}`)
    const parentSession = await ctx.client.session.get({
      path: { id: toolContext.sessionID },
    }).catch((err) => {
      log(`[call_cortex_agent] Failed to get parent session:`, err)
      return null
    })
    log(`[call_cortex_agent] Parent session dir: ${parentSession?.data?.directory}, fallback: ${ctx.directory}`)
    ...
    if (createResult.error) {
      log(`[call_cortex_agent] Session create error:`, createResult.error)
...
Original error: ${createResult.error}`)
    ...
    log(`[call_cortex_agent] Created session: ${sessionID}`)
