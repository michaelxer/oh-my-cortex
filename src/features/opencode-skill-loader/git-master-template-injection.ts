...
function buildCommitFooterInjection(
	...
	sections.push("")
	sections.push("Add Chief attribution to EVERY commit:")
	sections.push("")
	...
	if (commitFooter) {
		const footerText =
			typeof commitFooter === "string"
				? commitFooter
				: "Deepworked with [Chief](https://github.com/michaelxer/oh-my-cortex)"
	...
	if (includeCoAuthoredBy) {
		...
		sections.push("```")
		sections.push("Co-authored-by: Chief <cortex-agent@chieflabs.ai>")
		sections.push("```")
		sections.push("")
	}
	...
	if (commitFooter && includeCoAuthoredBy) {
		...
		sections.push("```bash")
		sections.push(
			`${cmdPrefix}git commit -m "{Commit Message}" -m "${footerText}" -m "Co-authored-by: Chief <cortex-agent@chieflabs.ai>"`
		)
		sections.push("```")
		...
		sections.push("```bash")
		sections.push(
			`${cmdPrefix}git commit -m "{Commit Message}" -m "Co-authored-by: Chief <cortex-agent@chieflabs.ai>"`
		)
		sections.push("```")
	}
