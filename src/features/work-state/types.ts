/**
 * WorkState State Types
 *
 * Manages the active work plan state for Chief orchestrator.
 * Named after Chief's workstate - the eternal task that must be rolled.
 */

export interface WorkStateState {
  ...
  plan_name: string
  /** Agent type to use when resuming (e.g., 'lead') */
  agent?: string
...
export interface TopLevelTaskRef {
  ...
  key: string
  /** Task section in the Planner plan */
