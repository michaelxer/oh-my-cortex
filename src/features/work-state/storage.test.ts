...
describe("work-state", () => {
  ...
  describe("readWorkStateState", () => {
    test("should return null when no workstate.json exists", () => {
      // given - no workstate.json file
      // when
    ...
    test("should return null for JSON null value", () => {
      //#given - workstate.json containing null
      const workstateFile = join(CHIEF_DIR, "workstate.json")
      writeFileSync(workstateFile, "null")
    ...
    test("should return null for JSON primitive value", () => {
      //#given - workstate.json containing a string
      const workstateFile = join(CHIEF_DIR, "workstate.json")
      writeFileSync(workstateFile, '"just a string"')
    ...
    test("should default session_ids to [] when missing from JSON", () => {
      //#given - workstate.json without session_ids field
      const workstateFile = join(CHIEF_DIR, "workstate.json")
    ...
    test("should default session_ids to [] when not an array", () => {
      //#given - workstate.json with session_ids as a string
      const workstateFile = join(CHIEF_DIR, "workstate.json")
      writeFileSync(workstateFile, JSON.stringify({
        active_plan: "/path/to/plan.md",
        started_at: "2026-01-01T00:00:00Z",
    ...
    test("should default session_ids to [] for empty object", () => {
      //#given - workstate.json with empty object
      const workstateFile = join(CHIEF_DIR, "workstate.json")
      writeFileSync(workstateFile, JSON.stringify({}))
    ...
    test("should read valid workstate state", () => {
      // given - valid workstate.json
    ...
    test("should default task_sessions to empty object when missing from JSON", () => {
      // given - workstate.json without task_sessions field
      const workstateFile = join(CHIEF_DIR, "workstate.json")
      writeFileSync(workstateFile, JSON.stringify({
        active_plan: "/path/to/plan.md",
        started_at: "2026-01-01T00:00:00Z",
  ...
  describe("writeWorkStateState", () => {
    test("should write state and create .cortex directory if needed", () => {
      ...
      // when
      const success = writeWorkStateState(TEST_DIR, state)
      const readBack = readWorkStateState(TEST_DIR)
  ...
  describe("appendSessionId", () => {
    ...
    test("should not crash when workstate.json has no session_ids field", () => {
      //#given - workstate.json without session_ids
      const workstateFile = join(CHIEF_DIR, "workstate.json")
      writeFileSync(workstateFile, JSON.stringify({
        active_plan: "/plan.md",
        started_at: "2026-01-01T00:00:00Z",
  ...
  describe("clearWorkStateState", () => {
    test("should remove workstate.json", () => {
      ...
      // when
      const success = clearWorkStateState(TEST_DIR)
      const result = readWorkStateState(TEST_DIR)
  ...
  describe("task session state", () => {
    test("should persist and read preferred session for a top-level plan task", () => {
      // given - existing workstate state
      const state: WorkStateState = {
        active_plan: "/plan.md",
        started_at: "2026-01-02T10:00:00Z",
        session_ids: ["session-1"],
    ...
    test("should overwrite preferred session for the same top-level plan task", () => {
      // given - existing workstate state with prior preferred session
      const state: WorkStateState = {
        active_plan: "/plan.md",
        started_at: "2026-01-02T10:00:00Z",
        session_ids: ["session-1"],
...
`)
  ...
  describe("getPlanName", () => {
    test("should extract plan name from path", () => {
      // given
      const path = "/home/user/.cortex/plans/project/my-feature.md"
      // when
  ...
  describe("createWorkStateState", () => {
    test("should create state with correct fields", () => {
      ...
      // when
      const state = createWorkStateState(planPath, sessionId)
    ...
    test("should include agent field when provided", () => {
      ...
      //#when - createWorkStateState is called with agent
      const state = createWorkStateState(planPath, sessionId, agent)

      //#then - state should include the agent field
      expect(state.agent).toBe("lead")
      expect(state.active_plan).toBe(planPath)
      expect(state.session_ids).toEqual([sessionId])
    ...
    test("should allow agent to be undefined", () => {
      ...
      //#when - createWorkStateState is called without agent
      const state = createWorkStateState(planPath, sessionId)
