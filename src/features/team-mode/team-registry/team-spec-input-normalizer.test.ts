...
describe("normalizeTeamSpecInput", () => {
  test("injects the caller as lead when no lead is specified", () => {
    ...
    const normalizedSpec = normalizeTeamSpecInput(rawSpec, {
      callerTeamLead: resolveCallerTeamLead("\u200BChief - Deepworker"),
    })
    ...
    expect(normalizedSpec).toMatchObject({
      leadAgentId: "lead",
      members: [
        { name: "lead", kind: "subagent_type", subagent_type: "chief" },
  ...
  test("keeps an explicit leadAgentId unchanged when the caller is eligible", () => {
    ...
    const rawSpec = {
      ...
      leadAgentId: "captain",
      members: [
        { kind: "subagent_type", name: "captain", subagent_type: "lead" },
  ...
  test("prefers isLead over the caller when both are present", () => {
    ...
    const rawSpec = {
      name: "alpha-team",
      members: [
        { kind: "subagent_type", name: "captain", subagent_type: "lead", isLead: true },
  ...
  test("throws a clear error when the caller is not eligible and no lead is specified", () => {
    ...
    const result = () => normalizeTeamSpecInput(rawSpec, {
      callerTeamLead: resolveCallerTeamLead("tracker"),
    })

    // then
    expect(result).toThrow("Caller agent tracker is not eligible as team lead; specify leadAgentId explicitly")
  })
