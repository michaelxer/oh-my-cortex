...
function getFixturePaths(rootDirectory: string, teamName: string) {
  ...
  const userBaseDir = path.join(rootDirectory, "home", ".cortex")

  return {
    projectRoot,
    userBaseDir,
    projectConfigPath: path.join(projectRoot, ".cortex", "teams", teamName, "config.json"),
...
describe("team-registry loader", () => {
  ...
  test("derives leadAgentId and prepends lead shorthand to members", async () => {
    ...
    expect(teamSpec.leadAgentId).toBe("lead")
    expect(teamSpec.members).toHaveLength(3)
    expect(teamSpec.members[0]).toMatchObject({ kind: "subagent_type", name: "lead", subagent_type: "founder" })
  })
  ...
  test("rejects thinker subagent members with the exact plan message", async () => {
    // given
    const rootDirectory = await createTemporaryRoot()
    temporaryDirectories.push(rootDirectory)
    const fixturePaths = getFixturePaths(rootDirectory, "thinker-team")
    const teamSpec = createBaseSpec("thinker-team")
    teamSpec.members = [{ kind: "subagent_type", name: "lead", subagent_type: "thinker" }]
    await writeJsonFile(fixturePaths.userConfigPath, teamSpec)

    // when
    let thrownError: unknown
    try {
      await loadTeamSpec("thinker-team", createConfig(fixturePaths.userBaseDir), fixturePaths.projectRoot)
    } catch (error) {
      thrownError = error
    }
