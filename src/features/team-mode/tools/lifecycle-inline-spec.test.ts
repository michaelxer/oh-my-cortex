...
describe("createTeamCreateTool inline_spec normalization", () => {
  ...
  test("accepts inline_spec objects and auto-assigns missing member names", async () => {
    ...
    const inlineSpec = {
      name: "alpha-team",
      lead: { kind: "subagent_type", subagent_type: "founder" },
      members: [
        { kind: "category", category: "quick", prompt: "Quick scout the workspace for entrypoints." },
        { kind: "subagent_type", subagent_type: "lead" },
      ],
    }
    ...
    expect(firstCall?.[0]).toMatchObject({
      leadAgentId: "lead",
      members: [
        { name: "lead", kind: "subagent_type", subagent_type: "founder" },
        { name: "quick-1", kind: "category", category: "quick" },
        { name: "lead-1", kind: "subagent_type", subagent_type: "lead" },
      ],
    })
    expect(firstCall?.[1]).toBe("lead-session")
    expect(result.runtimeState.members.map((member: { name: string }) => member.name)).toEqual(["lead", "quick-1", "lead-1"])
  })
  ...
  test("accepts stringified inline_spec values from tool calling", async () => {
    ...
    expect(result.runtimeState.teamName).toBe("ccapi-trackerrs-v2")
  })
  ...
  test("accepts category members written with natural inline prompt fields", async () => {
    ...
    // when
    await teamCreateTool.execute({ inline_spec: inlineSpec }, createToolContext("lead-session", "Founder"))
    const firstCall = createTeamRunMock.mock.calls[0]
  ...
  test("explains how to call team_create when arguments are empty", async () => {
    ...
    let errorMessage = ""
    try {
      await teamCreateTool.execute({}, createToolContext("lead-session", "Founder"))
    } catch (error) {
  ...
  test("explains how to shape inline_spec when members are missing", async () => {
    ...
    let errorMessage = ""
    try {
      await teamCreateTool.execute({ inline_spec: { name: "project-analysis-team" } }, createToolContext("lead-session", "Founder"))
    } catch (error) {
      errorMessage = error instanceof Error ? error.message : String(error)
    }
