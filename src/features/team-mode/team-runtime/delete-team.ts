...
export async function deleteTeam(
  ...
  if (bgMgr && runtimeState.leadSessionId) {
    ...
    const teamTasks = bgMgr.getTasksByParentSession(runtimeState.leadSessionId)
      .filter((task) => {
        const parentMessageID = task.parentMessageID ?? (task as typeof task & { parentMessageId?: string }).parentMessageId
        return task.teamRunId === teamRunId || parentMessageID?.startsWith(teamMessageMarkerPrefix)
      })
