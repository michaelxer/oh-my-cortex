...
describe("resolveMember", () => {
  ...
  test("strips workerModel before resolving category members so each declared category keeps its own model", async () => {
    // given
    const member = {
      backendType: "in-process",
      isActive: true,
    ...
    const ctxWithWorkerOverride: ExecutorContext = {
      ...createExecutorContext(),
    ...
    // when
    await resolveMember(member, ctxWithWorkerOverride, "ultrabrain, deep")

    // then
    const [, executorCtxArg] = resolveCategoryExecutionMock.mock.calls[0]
    expect(executorCtxArg.workerModel).toBeUndefined()
  })
  ...
  test("routes subagent members through resolveSubagentExecution", async () => {
    ...
    // when
    const result = await resolveMember(member, createExecutorContext(), "deep, quick", "founder")
    ...
    expect(result.agentToUse).toBe("lead")
    expect(result.systemContent).toBe("resolved-system-content")
  })
