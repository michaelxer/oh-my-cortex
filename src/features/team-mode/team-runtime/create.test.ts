...
const { createTeamRun: createTeamRunImpl, TeamRunCreateError } = await import("./create")
...
describe("createTeamRun", () => {
  ...
  test("spawns 3 members through BackgroundManager.launch without direct session creation", async () => {
    ...
    // when
    const runtimeState = await createTeamRunForTest(createSpec(3), "lead-session", context, createConfig(baseDir), manager)
  ...
  test("registers a member session as soon as launch reports the real sessionId", async () => {
    ...
    // when
    const runtimeState = await createTeamRunForTest(createSpec(1), "lead-session", createContext(baseDir, manager), createConfig(baseDir), manager)
  ...
  test("persists the resolved subagent_type and model on each spawned runtime member", async () => {
    ...
    // when
    const runtimeState = await createTeamRunForTest(createSpec(3), "lead-session", createContext(baseDir, manager), createConfig(baseDir), manager)
  ...
  test("member prompt only documents member-safe communication tools", async () => {
    ...
    // when
    await createTeamRunForTest(createSpec(1), "lead-session", createContext(baseDir, manager), createConfig(baseDir), manager)
  ...
  test("rolls back launched members in reverse order when a later spawn fails", async () => {
    ...
    // when
    const result = createTeamRunForTest(createSpec(4), "lead-session", createContext(baseDir, manager), createConfig(baseDir), manager)
  ...
  test("removes all created worktrees when spawn fails after worktree creation", async () => {
    ...
    // when
    try {
      await createTeamRunForTest(spec, "lead-session", createContext(baseDir, manager), createConfig(baseDir), manager)
      throw new Error("expected createTeamRun to reject")
    } catch (error) {
      expect(error).toBeInstanceOf(TeamRunCreateError)
    }
  ...
  test("returns the existing runtime on repeated calls with the same spec and lead session", async () => {
    ...
    // when
    const firstRuntime = await createTeamRunForTest(spec, "lead-session", context, createConfig(baseDir), manager)
    const secondRuntime = await createTeamRunForTest(spec, "lead-session", context, createConfig(baseDir), manager)
  ...
  test("never exceeds max_parallel_members while spawning", async () => {
    ...
    // when
    await createTeamRunForTest(createSpec(8), "lead-session", createContext(baseDir, manager), createConfig(baseDir, 4), manager)
  ...
  test("reuses the caller session for the lead when the lead matches the caller agent", async () => {
    ...
    // when
    const runtimeState = await createTeamRunForTest(
      spec,
      "lead-session",
  ...
  test("persists the reused caller lead's subagent_type so live deliveries can pin it", async () => {
    ...
    expect(leadMember?.sessionId).toBe("ses_caller_founder")
    expect(leadMember?.subagent_type).toBe("founder")
  ...
  test("reuses the caller session for the lead even when the lead subagent_type differs", async () => {
    ...
    const spec: TeamSpec = {
      ...
      leadAgentId: "captain",
      members: [
        { kind: "subagent_type", name: "captain", subagent_type: "lead", backendType: "in-process", isActive: true },
        { kind: "category", name: "member-1", category: "quick", prompt: "prompt-1", backendType: "in-process", isActive: true },
      ],
    }
