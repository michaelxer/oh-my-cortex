...
import { resolveMember as resolveMemberDefault } from "./resolve-member"
...
type CreateTeamRunDeps = {
  resolveMember?: typeof resolveMemberDefault
}
...
async function waitForTaskSessionId(bgMgr: BackgroundManager, task: BackgroundTask, deadlineAt: number): Promise<string> {
  let sessionId = task.sessionID ?? (task as BackgroundTask & { sessionId?: string }).sessionId
  while (!sessionId) {
    ...
    sessionId = updatedTask?.sessionID ?? (updatedTask as BackgroundTask & { sessionId?: string } | undefined)?.sessionId
...
export async function createTeamRun(
  ...
  deps: CreateTeamRunDeps = {},
): Promise<RuntimeState> {
  const resolveMember = deps.resolveMember ?? resolveMemberDefault
