...
function translateMemberError(
  ...
  if (input.kind === "subagent_type" || (!hasKind && hasSubagentType)) {
    ...
    if (typeof input.subagent_type !== "string" || !agentEligibilityRegistry[input.subagent_type]) {
      return new MemberValidationError(
        `Unknown subagent_type '${subagentType}'. Available eligible agents: chief, lead, worker, founder (conditional). Use delegate-task for read-only agents like thinker, researcher, tracker, reviewer, critic, spotter.`,
        name,
        "unknown-subagent",
      )
    }
