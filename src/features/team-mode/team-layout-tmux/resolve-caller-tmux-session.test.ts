...
async function createTmuxStub(options: { stdout: string; windowStdout?: string; exitCode: number }): Promise<TmuxStub> {
	...
	const tmuxPath = path.join(directory, process.platform === "win32" ? "tmux.cmd" : "tmux")
	const nodeScriptPath = path.join(directory, "tmux-stub.cjs")
	if (process.platform === "win32") {
		const nodeScript = [
			...
			`const windowStdout = ${JSON.stringify(options.windowStdout ?? options.stdout)}`,
			"const usesWindowFormat = args.includes('#{session_name}:#{window_index}')",
			"process.stdout.write(usesWindowFormat ? windowStdout : stdout)",
			`process.exit(${options.exitCode})`,
		].join("\n")
		const script = `@echo off\r\nnode "${nodeScriptPath}" %*\r\n`
		await writeFile(nodeScriptPath, nodeScript)
