...
describe("team-layout-tmux", () => {
  ...
  test("#given ownedSession=true, targetSessionId='omx-team-xyz' #when removeTeamLayout runs #then kill-session is called with -t omx-team-xyz (legacy behavior preserved)", async () => {
    // given
    const { removeTeamLayout } = await loadLayoutModule()
    ...
    // then
    const commands = getCommands()
    expect(commands).toContainEqual(["kill-session", "-t", "omx-team-xyz"])
  })
