...
const { createTeamRun: createTeamRunImpl } = await import("./team-runtime/create")
...
function createTeamRunForTest(
  ...args: Parameters<typeof createTeamRunImpl>
): ReturnType<typeof createTeamRunImpl> {
  return createTeamRunImpl(args[0], args[1], args[2], args[3], args[4], args[5], args[6], {
...
describe("team-mode integration", () => {
  test("C-10.1 creates a single-member echo team, delivers mail, surfaces unread status, and deletes runtime", async () => {
    ...
    const runtime = await createTeamRunForTest(createSpec("echo-team", "echo", [{ kind: "subagent_type", name: "echo", subagent_type: "lead", backendType: "in-process", isActive: true }]), "ses_lead", createContext(baseDir, manager, new Set(["ses_lead"])), config, manager)
  ...
  test("C-10.2 runs a 2-member pipeline where worker claims and completes a lead-created task", async () => {
    ...
    const config = createConfig(baseDir)
    const manager = createManager()
    const runtime = await createTeamRunForTest(createSpec("pipeline-team", "lead", [
      { kind: "subagent_type", name: "lead", subagent_type: "founder", backendType: "in-process", isActive: true },
      { kind: "subagent_type", name: "worker", subagent_type: "lead", backendType: "in-process", isActive: true },
  ...
  test("C-10.3 resumes alive teams, orphans dead leads, fails stuck creating teams, and cleans deleting runs", async () => {
    ...
    const aliveRuntime = await createTeamRunForTest(createSpec("alive-team", "lead", [{ kind: "subagent_type", name: "lead", subagent_type: "founder", backendType: "in-process", isActive: true }]), "ses_alive", context, config, manager)
    const deadRuntime = await createTeamRunForTest(createSpec("dead-team", "lead", [{ kind: "subagent_type", name: "lead", subagent_type: "lead", backendType: "in-process", isActive: true }]), "ses_dead", context, config, manager)
    const stuckRuntime = await createTeamRunForTest(createSpec("stuck-team", "lead", [{ kind: "subagent_type", name: "lead", subagent_type: "lead", backendType: "in-process", isActive: true }]), "ses_stuck", context, config, manager)
    const deletingRuntime = await createTeamRunForTest(createSpec("deleting-team", "lead", [{ kind: "subagent_type", name: "lead", subagent_type: "lead", backendType: "in-process", isActive: true }]), "ses_delete", context, config, manager)
  ...
  test("C-10.5 end-to-end: createTeamRun persists category-aware routing and team_send_message reapplies it on promptAsync", async () => {
    ...
    const runtime = await createTeamRunForTest(createSpec("msg-team", "lead", [
      { kind: "subagent_type", name: "lead", subagent_type: "founder", backendType: "in-process", isActive: true },
  ...
  test("C-10.4 keeps member spawn concurrency within max_parallel_members", async () => {
    ...
    // when
    await createTeamRunForTest(createSpec("parallel-team", "lead", [
      { kind: "subagent_type", name: "lead", subagent_type: "founder", backendType: "in-process", isActive: true },
      { kind: "subagent_type", name: "worker-a", subagent_type: "lead", backendType: "in-process", isActive: true },
      { kind: "subagent_type", name: "worker-b", subagent_type: "lead", backendType: "in-process", isActive: true },
