...
describe("migrations sidecar", () => {
  ...
  beforeEach(() => {
    workdir = mkdtempSync(join(tmpdir(), "omx-migrations-sidecar-"))
  })
  ...
  describe("getSidecarPath", () => {
    test("appends .migrations.json to the config path", () => {
      expect(getSidecarPath("/home/user/.config/opencode/oh-my-cortex.json")).toBe(
        "/home/user/.config/opencode/oh-my-cortex.json.migrations.json",
      )
    })

    test("works for jsonc configs too", () => {
      expect(getSidecarPath("/home/user/oh-my-cortex.jsonc")).toBe(
        "/home/user/oh-my-cortex.jsonc.migrations.json",
      )
    })
  })
  ...
  describe("writeAppliedMigrations", () => {
    ...
    test("creates parent directories if they do not exist yet", () => {
      ...
      const configPath = join(nested, "oh-my-cortex.json")
    ...
    test("round-trips via readAppliedMigrations", () => {
      const configPath = join(workdir, "oh-my-cortex.jsonc")
