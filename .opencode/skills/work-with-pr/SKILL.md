...
### Sync .cortex state back to main repo

Before removing the worktree, copy `.cortex/` state back. When `.cortex/` is gitignored, files written there during worktree execution are not committed or merged — they would be lost on worktree removal.

```bash
# Sync .cortex state from worktree to main repo (preserves task state, plans, notepads)
if [ -d "$WORKTREE_PATH/.cortex" ]; then
  mkdir -p "$ORIGINAL_DIR/.cortex"
  cp -r "$WORKTREE_PATH/.cortex/"* "$ORIGINAL_DIR/.cortex/" 2>/dev/null || true
fi
```
