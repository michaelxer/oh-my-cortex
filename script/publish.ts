...
const PACKAGE_NAME = "oh-my-cortex"
...
console.log("=== Publishing oh-my-cortex (multi-package) ===\n")
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `oh-my-cortex-${platform}`
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  const team = ["actions-user", "github-actions[bot]", "michaelxer"]

  try {
    const compare =
      await $`gh api "/repos/michaelxer/oh-my-cortex/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
async function gitTagAndRelease(newVersion: string, notes: string[]): Promise<void> {
  ...
  await $`git add package.json assets/oh-my-cortex.schema.json`
  for (const platform of PLATFORM_PACKAGES) {
